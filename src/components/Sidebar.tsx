  ...
  return <aside className={`bg-slate-900 text-white border-r border-slate-800 flex flex-col z-30 shrink-0 select-none print:hidden ${isMobileMenu ? 'w-72 max-w-[85vw]' : isCollapsed ? 'w-18' : 'w-64'}`}>
    <div>
      <div className="p-4 border-b border-slate-800 flex items-center justify-between">
        {(!isCollapsed || isMobileMenu) ? <div className="flex items-center gap-3 min-w-0"><div className="brand-mark w-9 h-9 rounded-lg bg-emerald-600 flex items-center justify-center shrink-0"><Scale className="w-5 h-5" /></div><div className="min-w-0"><div className="text-sm font-bold truncate">Metrology Desk</div><div className="text-[10px] text-slate-400 font-mono tracking-wide">LM / INSPECTION OS</div></div></div> : <div className="brand-mark w-9 h-9 mx-auto rounded-lg bg-emerald-600 flex items-center justify-center"><Scale className="w-5 h-5" /></div>}
